<template>
  <MacroTaskAccordionTemplate
    ...
    :index="index"
    icon="mdi-repeat"
    :title="$t('macro.presets.conditions.forLoop')"
    :detail="loopTitleDetail"
...
export default {
  ...
  computed: {
    loopTitleDetail(): string {
      const key = String(this.loopData.key ?? 'item').trim() || 'item'
      if (this.loopMode === 'array') {
        const source = String(this.loopData.data ?? '').trim()
        const count = Array.isArray(this.loopData.values) ? this.loopData.values.length : 0
        return source ? `${key} · ${source}` : (count ? `${key} · ${count}` : key)
      }
      const from = this.loopData.from ?? 1
      const to = this.loopData.to ?? 10
      return `${key} · ${from} → ${to}`
    },
    loopData(): any {
      const task = (this.item as any).task
