<template>
  ...
  >
    <v-row density="comfortable">
      ...
      <v-col cols="12" md="6">
        <v-select v-model="task.data.end" :items="endActionOptions" item-title="title" item-value="value" :label="$t('macro.final.timer.endAction')" prepend-inner-icon="mdi-flag-checkered" variant="outlined" hide-details="auto" />
      </v-col>
      <v-col cols="12" md="6">
        <v-combobox v-model="task.data.finished_macro" :items="macroOptions" :label="$t('macro.final.timer.finishedMacro')" :hint="$t('macro.final.timer.optionalMacroThatRunsWhenTheTimerReaches0')" persistent-hint clearable prepend-inner-icon="mdi-playlist-play" density="comfortable" variant="outlined" hide-details="auto" @update:model-value="normalizeFinishedMacro" />
      </v-col>
    </v-row>
  </MacroTaskAccordionTemplate>
</template>
...
export default {
  name: 'MacroTimerStartTaskAccordion',
  ...
  computed: {
    ...
    task(): any {
      ...
      if (!Number.isFinite(Number(task.data.time))) task.data.time = 10
      task.data.unit ||= 'seconds'
      task.data.end ||= 'blink'
      if (task.data.finished_macro === null) delete task.data.finished_macro
      return task
    },
    ...
    endActionOptions(): Array<{ title: string; value: string }> {
      return [
    ...
    macroOptions(): string[] {
      const macros = this.appStore.getMacros ?? {}
      if (Array.isArray(macros)) {
        return macros.map((item: any) => typeof item === 'string' ? item : item?.name).filter(Boolean).map(String).sort((a: string, b: string) => a.localeCompare(b))
      }
      return Object.keys(macros).filter(Boolean).sort((a, b) => a.localeCompare(b))
    },
  },
  created() { this.task },
  methods: {
    normalizeFinishedMacro(value: string | null) {
      ...
      if (!macroName) delete this.task.data.finished_macro
      else this.task.data.finished_macro = macroName
    },
  },
}
</script>
