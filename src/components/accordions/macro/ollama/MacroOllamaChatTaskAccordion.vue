...
export default {
  ...
  computed: {
    ollamaTitleDetail(): string {
      const messages = Array.isArray(this.task?.data?.messages) ? this.task.data.messages : []
      const message = messages.find((entry: any) => String(entry?.content ?? '').trim())
      if (!message) return ''
