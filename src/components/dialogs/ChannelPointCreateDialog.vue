<template>
  ...
  >
    <v-card color="grey-darken-4" class="channel-point-dialog">
      ...
      <v-card-text class="py-3 px-0">
        ...
        <div class="option-card-grid px-3 mb-4">
          <v-card variant="tonal" class="option-card pa-4">
            <div class="text-subtitle-2 mb-3">{{ $t('dialogs.channelPointCreateDialog.general') }}</div>
            <v-text-field v-model="form.name" :label="$t('dialogs.channelPointCreateDialog.name')" density="comfortable" variant="outlined" hide-details />
          </v-card>

          <v-card variant="tonal" class="option-card pa-4">
            <div class="text-subtitle-2 mb-3">Options</div>
            <v-row density="comfortable">
              <v-col cols="12" sm="6"><v-switch v-model="form.enable_default" :label="$t('dialogs.channelPointCreateDialog.enableDefault')" color="primary" density="comfortable" hide-details inset /></v-col>
              <v-col cols="12" sm="6"><v-switch v-model="form.auto_accept" :label="$t('dialogs.channelPointCreateDialog.autoAccept')" color="primary" density="comfortable" hide-details inset /></v-col>
              <v-col cols="12" sm="6"><v-switch v-model="form.strip_emotes" :label="$t('dialogs.channelPointCreateDialog.stripEmotes')" color="primary" density="comfortable" hide-details inset /></v-col>
              <v-col cols="12" sm="6"><v-switch v-model="form.input_required" :label="$t('dialogs.channelPointCreateDialog.inputRequired')" color="primary" density="comfortable" hide-details inset /></v-col>
            </v-row>
          </v-card>

          <v-card variant="tonal" class="option-card pa-4">
            <div class="text-subtitle-2 mb-3">Interaction</div>
            <v-switch v-model="form.bypass_interaction_queue" :label="$t('dialogs.channelPointCreateDialog.bypassInteractionQueue')" color="primary" density="comfortable" hide-details inset />
          </v-card>
        </div>
          ...
          <v-expansion-panel value="asset">
            <v-expansion-panel-title>
              <div class="d-flex align-center ga-2 min-width-0">
                ...
                <span class="text-truncate">{{ $t('dialogs.channelPointCreateDialog.asset') }}</span>
              </div>
            </v-expansion-panel-title>
          ...
          <v-expansion-panel value="macro">
            <v-expansion-panel-title>
              <div class="d-flex align-center ga-2 min-width-0">
                ...
                <span class="text-truncate">{{ $t('dialogs.channelPointCreateDialog.macro') }}</span>
              </div>
            </v-expansion-panel-title>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" @click="$emit('update:modelValue', false)">{{ $t('dialogs.channelPointCreateDialog.cancel') }}</v-btn>
...
export default {
  ...
  computed: {
    title(): string {
      return this.$t('channelPoints.create')
    },
