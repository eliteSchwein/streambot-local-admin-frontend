  ...
  methods: {
    async open() {
      ...
      this.form = {
        ...
        single_use: ['user', 'global'].includes(command.single_use ?? command.singleUse) ? (command.single_use ?? command.singleUse) : 'none',
        user_list_mode: ['blacklist', 'whitelist'].includes(command.user_list_mode ?? command.userListMode) ? (command.user_list_mode ?? command.userListMode) : 'none',
        users: this.toArray(command.users ?? command.user_list ?? command.userList).map((user: any) => String(user ?? '').replace(/^@+/, '').trim()).filter(Boolean),
      ...
      await Promise.all([
        ...
        this.loadMacro(macroName).then((content: string) => { this.macroContent = content }),
      ])

      await this.$nextTick()
      ;(this.$refs.macroAccordion as any)?.setContent?.(this.macroContent, macroName)
    },
    ...
    async loadMacro(name: string) {
      const payload = { name, path: `${name}.yaml`, file: `${name}.yaml` }
      try {
      ...
      return this.defaultMacroContent(name)
    },
    ...
    async save() {
      ...
      try {
        ...
        // The parent switches its `loading` prop on synchronously when the
        // save event is emitted. This internal state covers the work that
        // happens before that event (notably saving the asset accordion), so
        // the button starts spinning immediately on click.
        this.savingInternal = false
      }
    },
  },
}
